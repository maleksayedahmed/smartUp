...
<div class="breadcrumb-wrapper">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="#">مميزات النظام  </a>
        </li>
    </ol>
</div>
...
@section('js')
    ...
    <script>
     ...
     <script type="text/javascript">
        ...
        var table = $('.yajra-datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('dashboard.get_all_systems_features',$id) }}",
            columns: [
        ...
        $(document).on('click', '#add_user', function (e) {
            ...
            var formData = new FormData($('#add_user_form')[0]);
                $.ajax({
                    ...
                    url: "{{route('dashboard.store_systems_features')}}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    cache: false,
        ...
        function do_update(){
            ...
            var formData = new FormData($('#edit_user_form')[0]);
                $.ajax({
                    type: 'post',
                    enctype: 'multipart/form-data',
                    url: "{{route('dashboard.update_systems_features')}}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    cache: false,
   ...
   <script>
        function do_delete(){
            ...
            $.ajax({
                type: 'post',
                url: "{{route('dashboard.destroy_systems_features')}}",
                data: formData,
                processData: false,
                contentType: false,
                success: function (data) {
